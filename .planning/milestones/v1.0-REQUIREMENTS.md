# Requirements Archive: v1.0 MVP — Trilingual B2B Catalog
...
- [x] **CAT-01**: Visitor can switch between Uzbek (Latin), Russian, and English site-wide; current locale reflected in URL
- [x] **CAT-02**: Visitor can browse the category tree via a persistent navigation (left nav or mega-menu)
- [x] **CAT-03**: Visitor sees a category listing page with products, filters, and result count
- [x] **CAT-04**: Visitor can filter products on a category page using faceted filters driven by the category's typed spec fields (number ranges, enum selections, boolean toggles)
- [x] **CAT-05**: Filter state is reflected in the URL (shareable/bookmarkable) via nuqs
- [x] **CAT-06**: Visitor sees a product detail page with: hero image, gallery, full fiztech-density spec tables grouped by category-defined spec groups, free-form extras section, manufacturer attribution, downloadable PDFs/datasheets/certificates, "Used in" applications section
- [x] **CAT-07**: All public pages are server-rendered (SSR or ISR) — HTML is populated on first byte, not client-fetched
- [x] **CAT-08**: Product detail pages emit Product JSON-LD; organization pages emit Organization JSON-LD; category pages emit CollectionPage + BreadcrumbList JSON-LD

### Search (SRCH)

- [x] **SRCH-01**: Visitor can perform a full-text search across product name, description, and spec text in the current locale
- [x] **SRCH-02**: Search results page returns products ranked by relevance; empty current-locale results fall back to another locale with a "shown in [other locale]" hint
- [x] **SRCH-03**: Search bar offers autocomplete suggestions as the user types (products matched by name/part-number)
- [x] **SRCH-04**: Exact part-number matches short-circuit to the product detail page
- [x] **SRCH-05**: Search index is rebuilt transactionally when a product (or its translations or spec values) is updated in admin

### Recipes and Industries (CONT)

- [x] **CONT-01**: Admin can CRUD rich-text articles ("recipes" / selection guides) with Tiptap editor — links, images (Cloudinary), tables, headings; translated across all three locales
- [x] **CONT-02**: Admin can CRUD industry-scenario pages (e.g., oil & gas, food processing, pharma) with translated content and linked recommended products
- [x] **CONT-03**: Visitor can browse and read recipes and industry pages in their current locale
- [x] **CONT-04**: Product detail pages show a "Used in" section listing the recipes and industry pages that reference the product
- [x] **CONT-05**: Admin can assign M:N relationships between products ↔ recipes and products ↔ industries
- [x] **CONT-06**: Recipes and industry pages emit TechArticle JSON-LD

### Manufacturers (MFG)

- [x] **MFG-01**: Visitor can view a manufacturers/partners index page listing all partner brands with logos
- [x] **MFG-02**: Visitor can view a manufacturer detail page showing brand description and products from that manufacturer
...
- [x] **SEO-01**: Every page emits `<link rel="alternate" hreflang>` tags for all three locales plus `x-default`
- [x] **SEO-02**: Every page has a per-locale canonical URL (not a master canonical)
- [x] **SEO-03**: Site emits per-locale XML sitemaps referenced from `robots.txt`
- [x] **SEO-04**: Fonts load via `next/font` with subsets `['latin', 'latin-ext', 'cyrillic']`; Uzbek Latin `oʻ`/`gʻ` (U+02BB) renders correctly across locales
- [x] **SEO-05**: Product images render through `<CldImage>` with responsive `sizes` attribute; LCP on product detail page passes Core Web Vitals on Slow 4G (emerging-market budget)
...
| MFG-01 | Phase 3 | Complete |
| MFG-02 | Phase 3 | Complete |
