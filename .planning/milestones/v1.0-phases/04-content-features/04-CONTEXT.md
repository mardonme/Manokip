...
- **D-03:** **Status + publishedAt dual-column lifecycle, mirroring Phase-2 D-11 / 02-13b.** `saveRecipe` / `saveIndustry` write `status` verbatim from input + never derive from `publishedAt`. Dedicated `publishRecipe` / `unpublishRecipe` (and industry equivalents) atomic dual-column writes (status + publishedAt in ONE SET clause). `deleteRecipe` / `deleteIndustry` audit before-delete inside tx; FK cascade drops translations + junction rows. All wrap `withAdminAction` from Phase 2 plan 02-04 (audit_log row per mutation). **Audit verbs reuse the generic `publish` / `unpublish` / `delete` / `update` / `create` AUDIT_ACTIONS** with `entity_type` set to `'recipe'` / `'industry'` (matches Phase 2 plan 02-13b lifecycle pattern verified at `src/lib/audit.ts` — no AUDIT_ACTIONS enum extension needed). All call typed `revalidateRecipe(id)` / `revalidateIndustry(id)` helpers (extends Phase 2 plan 02-05 helper set) AFTER tx.commit (Pitfall #2).
...
- **D-05:** **Tiptap v3.22.5 (not v2 — STACK.md note is stale; researcher verified npm registry).** All 11 Tiptap packages release in lockstep — pin all to the exact same v3 minor across the project. Locked extensions: `@tiptap/starter-kit` (paragraph, headings 1–4, bold, italic, lists, blockquote, code, hard break) + `@tiptap/extension-link` (with target=\_blank rel=nofollow on external) + `@tiptap/extension-image` (extended below in D-06) + `@tiptap/extension-table` (+ table-row + table-cell + table-header) + `@tiptap/static-renderer` (server-side static renderer for public pages). Headings cap at H4 (H1 reserved for the page title outside the editor). No code-block syntax highlighting in v1 (low value for industrial pressure articles; defer).

- **D-06:** **Tiptap image upload uses the existing Cloudinary signed direct upload flow** wired into the editor's drop / paste / toolbar handlers. The signed upload returns a public_id; the Tiptap image node stores the public_id in a custom `data-public-id` attribute (NOT the full URL) so that public rendering goes through `<CldImage>` with the same responsive `sizes` strategy as Phase 3. Reuse `/api/cloudinary/sign` from Phase 1 plan 01-06 + the widget-paramsToSign protocol from Phase 2 plan 02-14 — no new server endpoint.

  **Custom Image extension shape:** `Image.extend({ addAttributes() { return { ...this.parent?.(), publicId: { default: null, parseHTML: el => el.getAttribute('data-public-id'), renderHTML: attrs => attrs.publicId ? { 'data-public-id': attrs.publicId } : {} } } } })`. Editor admin renders via Cloudinary widget on insertion; public RSC overrides `nodeMapping.image` in `@tiptap/static-renderer` to emit `<img>` with src from `getCldImageUrl({ src: publicId, format: 'auto', quality: 'auto' })`.
...
- **D-08:** **Tiptap JSON → HTML on the server via `@tiptap/static-renderer/pm/html-string` (RSC, no hydration cost).** Researcher verified by source-reading `dist/`: zero React imports, sync `renderToHTMLString({ extensions, content, options }): string` API, **built-in `escapeHTML` + `escapeHTMLAttribute` for all text + attributes** — the locked extension array IS the XSS allowlist (no DOMPurify needed). The renderer throws on unknown node types unless `unhandledNode` option is set; production config sets `unhandledNode: () => null` so unrecognised input falls back to plain text rather than throwing. Public bodies render inside a `prose prose-slate` Tailwind-typography wrapper for readable defaults. Admin Tiptap editor MUST set `immediatelyRender: false` per Tiptap v3 + Next 16 RSC compatibility (researcher pitfall P4-1).
...
- **D-10:** **Both recipes and industries emit TechArticle JSON-LD** (recipe is the canonical TechArticle subtype; industry pages are technically a stretch — schema.org scopes TechArticle to how-to/step-by-step/specs and `Article` is a closer fit for vertical landing pages — see researcher P4-4 / A1). Field set: `headline`, `image`, `datePublished`, `dateModified`, `author` = manometr Organization, `publisher` = same Organization, `inLanguage` = current locale, `mainEntityOfPage`. Implementation: extend `src/lib/jsonld.ts` from Phase 3 with `techArticleJsonLd(article, locale, mentions?)` helper. Optional `mentions` array references linked products as `Product` sub-objects (no `offers`, consistent with Phase 3 D-08) — gives Google a knowledge-graph signal connecting article → product. **Manual gate added to Phase 4 verification:** validate both recipe and industry JSON-LD against Google Rich Results Test on a Vercel preview; if industry pages fail TechArticle validation post-launch, downgrade to `Article` in a v1.1 follow-up plan (NOT a blocker for v1 ship). XSS escape pattern (`\\u003c`) reused from Phase 3 D-09.
