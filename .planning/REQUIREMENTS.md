...
| ADMIN-06 | Phase 2 | Partial (02-13a — data tier shipped: productInsertSchema (3 locales × name+slug+shortDesc?+longDesc? + spec values typed long-table + extras + status enum + per-field MT flags) + saveProduct 5-step atomic transaction with W7 refusal-to-elevate USE_PUBLISH_ACTION; 7/7 live-Neon specs lock create/update/spec-replace-on-save/mtFlags-replace-on-save/refusal/rollback/duplicate. UI + lifecycle (publishProduct/unpublishProduct/deleteProduct + product editor pages) land in 02-13b) |
| ADMIN-07 | Phase 2 | Pending |
| ADMIN-08 | Phase 2 | Partial (02-13a — duplicateProduct Server Action shipped: full clone with status forced to 'draft', publishedAt forced null, sku NOT cloned (unique column), per-locale slug suffixed `-copy`, spec values + their per-locale text translations + MT flags cloned 1-for-1, audit action='duplicate_product' with before.sourceId + after.id; 1 live-Neon spec locks the contract. "Duplicate this product" UI button lands in 02-13b) |
| ADMIN-09 | Phase 2 | Partial (02-01 — productTranslationFieldFlags sibling table landed: PK (product_id, locale, field_name) + machine_translated boolean + compound FK to product_translations ON DELETE CASCADE. 02-13a — write path landed: productInsertSchema accepts mtFlags as per-locale Record<fieldName, boolean>; saveProduct does replace-on-save inside the atomic tx (DELETE all flags for productId then INSERT only truthy entries); duplicateProduct clones flags 1-for-1; 1 live-Neon spec locks replace-on-save semantics. MT toggle UI + the visual flag in the editor land in 02-13b) |
