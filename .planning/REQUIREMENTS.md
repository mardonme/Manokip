...
| CONT-03 | Phase 4 | Partial — recipes complete, industries pending (04-02 + 04-03 — server-side renderTiptapToHtml + nodeMapping.image override + getRecipeBySlug/getIndustryBySlug with Phase-3 D-05 fallback cascade + usedFallbackLocale flag + findPublishedRecipes/Industries list helpers; 04-04 — 4 Playwright fixme specs locking visitor flow; 04-09 — recipe public surface complete: /[locale]/recipes RSC index page with RecipeCard grid + cached findPublishedRecipes(locale) fetch + per-locale canonical/hreflang via buildAlternates({pathPrefix:'/recipes'}); /[locale]/recipes/[slug] RSC detail page with renderTiptapToHtml(body) inside <article><div className='article-body prose prose-slate max-w-none'> with prose styling scoped to body div per Plan Rule 2 + LocaleFallbackBanner ('use client' with stateful dismiss button per Phase 3 D-05) mounted when usedFallbackLocale != null with localized message from public.localeFallback.recipe.<fallbackLocale> next-intl namespace + CldImage hero priority w=1200 + h1+lede outside prose; messages files extended with 3 namespaces public.localeFallback.recipe + public.localeFallback.industry + public.recipes.index across uz/ru/en — industry namespace shipped here so 04-10 inherits zero-change wiring; full closure when industries mirror lands in 04-10 and Playwright recipe-detail + industry-detail specs flip GREEN in 04-12) |
