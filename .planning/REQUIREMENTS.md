...
| CONT-01 | Phase 4 | Partial (04-02 — Tiptap v3.22.5 stack + TIPTAP_EXTENSIONS shared array + CloudinaryImage extension + tiptap-render server helper landed; 04-04 — 7 it.skip RED specs locking saveRecipe/publishRecipe/unpublishRecipe/deleteRecipe contracts in tests/actions/recipes.test.ts + 2 jsdom RED specs in tests/components/recipe-form.test.tsx + 2 Playwright fixme specs in tests/e2e/recipe-detail.spec.ts + 1 admin-recipe-form smoke spec; recipe Server Actions land in 04-05 GREEN-flipping the action stubs; admin UI lands in 04-07 GREEN-flipping the form stubs) |
| CONT-02 | Phase 4 | Partial (04-02 — same Tiptap foundation; 04-04 — 7 it.skip RED specs in tests/actions/industries.test.ts + 2 jsdom RED specs in tests/components/industry-form.test.tsx + 2 Playwright fixme specs in tests/e2e/industry-detail.spec.ts + 1 admin-industry-form smoke spec; industry Server Actions land in 04-06 GREEN-flipping the action stubs; admin UI lands in 04-08 GREEN-flipping the form stubs) |
...
| CONT-04 | Phase 4 | Partial (04-01 schema + 04-03 lib — getUsedInForProduct reading productUsedInView with cap-at-6 per D-09 + revalidateUsedIn cache-tag fan-out shipped; 04-04 — 2 Playwright fixme specs in tests/e2e/used-in-section.spec.ts (visible with cross-links + hidden when 0 cross-links) + Used-in revalidate fan-out OLD ∪ NEW linkedProductIds union locked in saveRecipe/Industry RED specs; UsedInSection RSC mounted on product detail page lands in 04-11 GREEN-flipping the e2e stubs) |
| CONT-05 | Phase 4 | Partial (04-04 — replace-on-save semantics for linkedProductIds locked in saveRecipe/saveIndustry RED specs (DELETE+INSERT replace, position field set, junction rows match input array) + LinkedProductsPicker jsdom RED stubs (filter + multi-select); seed-content fixture orchestrator with seedProductRecipes + seedProductIndustries auto-incrementing junction inserts; M:N junction Server Action implementation lands in 04-05 + 04-06 GREEN-flipping the action stubs; admin UI picker lands in 04-07) |
