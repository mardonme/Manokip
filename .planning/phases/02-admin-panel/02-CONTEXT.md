...
- **D-15:** **Session timeout (ADMIN-01) enforced in `proxy.ts` (middleware) + Auth.js `maxAge` as backstop.** On every `/[locale]/admin/*` request, the middleware:
  ...
  3. Rejects if `sessions.created_at < now() - '7d'` (absolute timeout — **new column** `sessions.created_at TIMESTAMPTZ NOT NULL DEFAULT now()` if the Auth.js adapter doesn't already provide it; verify at planning time).
